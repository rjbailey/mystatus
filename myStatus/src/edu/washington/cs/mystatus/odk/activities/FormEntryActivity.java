...
import edu.washington.cs.mystatus.odk.listeners.AdvanceToNextListener;
...
import edu.washington.cs.mystatus.odk.logic.FormController.FailedConstraint;
...
import edu.washington.cs.mystatus.odk.tasks.SaveToDiskTask;
import edu.washington.cs.mystatus.odk.utilities.FileUtils;
import edu.washington.cs.mystatus.odk.utilities.MediaUtils;
import edu.washington.cs.mystatus.odk.views.ODKView;
import edu.washington.cs.mystatus.odk.widgets.QuestionWidget;
...
public class FormEntryActivity extends Activity implements AnimationListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (data instanceof FormLoaderTask) {
			...
			if (!newForm) {
				if (MyStatus.getInstance().getFormController() != null) {
					refreshCurrentView();
				} else {
			...
			MyStatus.getInstance().setFormController(null);
...
//												.getColumnIndex(InstanceColumns.INSTANCE_FILE_PATH));
					...
					try {
						...
						if (c.getCount() != 1) {
							...
							File cacheDir = new File(MyStatus.CACHE_PATH);
							...
							for (int i = 0; i < files.length; ++i) {
								...
								File instanceDir = new File(
										MyStatus.INSTANCES_PATH + File.separator
												+ instanceDirName);
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		case IMAGE_CAPTURE:
			...
			// file
			File fi = new File(MyStatus.TMPFILE_PATH);
				...
				.setSingleChoiceItems(languages, selected,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int whichButton) {
								FormController formController = MyStatus
...
//		}
	...
	public void loadingComplete(FormLoaderTask task) {
		...
		if (formController.getInstancePath() == null) {
			...
			String file = mFormPath.substring(mFormPath.lastIndexOf('/') + 1,
					mFormPath.lastIndexOf('.'));
			String path = MyStatus.INSTANCES_PATH + File.separator + file + "_"
					+ time;
	...
	private void sendSavedBroadcast() {
		Intent i = new Intent();
		i.setAction("edu.washington.cs.mystatus.FormSaved");
